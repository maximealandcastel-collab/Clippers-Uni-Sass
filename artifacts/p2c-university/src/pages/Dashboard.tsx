...
import {
  ...
  Download, Package, Film, Image as ImageIcon, ChevronDown, ChevronUp, X, Zap, Target, Megaphone
} from "lucide-react";
import { SiTiktok, SiInstagram, SiYoutube, SiSnapchat } from "react-icons/si";
import { campaigns, campaignBriefs } from "./Campaigns";
...
export default function Dashboard() {
  ...
  const [pickerBriefCampaign, setPickerBriefCampaign] = useState<typeof campaigns[0] | null>(null);
  const [pickerApplying, setPickerApplying] = useState<number | null>(null);
  const [pickerApplied, setPickerApplied] = useState<number[]>([]);
  ...
  const handlePickerApply = async (campaign: typeof campaigns[0]) => {
    if (!token) return;
    setPickerApplying(campaign.id);
    try {
      ...
      if (res.ok || res.status === 409) {
        setPickerApplied(prev => [...prev, campaign.id]);
      }
    } catch {}
    setPickerApplying(null);
  };
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white rounded-2xl w-full max-w-2xl my-4 shadow-2xl overflow-hidden">
            ...
            {!pickerBriefCampaign ? (
              <>
                {/* Picker header */}
                <div className="bg-gradient-to-br from-[#0a0a0a] to-[#1a1a1a] p-7 text-white text-center relative">
                  ...
                  <div className="text-primary text-xs font-bold uppercase tracking-widest mb-1">Stripe Connected ✓</div>
                  <h2 className="text-2xl font-bold mb-2">Start making money now</h2>
                  <p className="text-white/60 text-sm">Pick a campaign, read the brief, and start creating content. Your first paycheck is closer than you think.</p>
                </div>

                {/* Campaign mini boxes */}
                <div className="p-6">
                  <div className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-4">Choose your first campaign</div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {campaigns.filter(c => !c.restricted).slice(0, 4).map(c => (
                      <button
                        key={c.id}
                        onClick={() => setPickerBriefCampaign(c)}
                        className="group text-left border-2 border-border hover:border-primary rounded-2xl p-4 transition-all hover:shadow-md hover:shadow-primary/10 bg-white"
                      >
                        <div className="flex items-center gap-3 mb-3">
                          <div className="w-10 h-10 rounded-xl overflow-hidden border border-border flex-shrink-0 bg-white">
                            {(c as any).logo ? (
                              <img src={(c as any).logo} alt={c.brand} className="w-full h-full object-cover" />
                            ) : (
                              <span className="w-full h-full flex items-center justify-center font-bold text-primary">{c.brand[0]}</span>
                            )}
                          </div>
                          <div className="min-w-0">
                            <div className="font-bold text-sm truncate">{c.brand}</div>
                            <div className="text-primary font-bold text-sm">{c.rate}</div>
                          </div>
                        </div>
                        <div className="text-xs text-muted-foreground truncate mb-3">{c.project}</div>
                        <div className="flex items-center justify-between">
                          <span className="text-xs bg-secondary px-2 py-1 rounded-md font-medium">{c.platform.split(" + ")[0]}</span>
                ...
                <div className="px-6 pb-6">
                  <button
                    ...
                    className="w-full py-3 border border-border rounded-xl text-sm font-semibold text-muted-foreground hover:bg-secondary transition-colors"
                  >
              ...
              <>
                <div className="bg-gradient-to-br from-primary to-primary/80 p-6 text-white relative">
                  ...
                  <div className="flex items-center gap-4 mb-3 pl-10">
                    <div className="w-12 h-12 rounded-xl overflow-hidden bg-white border-2 border-white/30 flex-shrink-0">
                      {(pickerBriefCampaign as any).logo ? (
                        <img src={(pickerBriefCampaign as any).logo} alt={pickerBriefCampaign.brand} className="w-full h-full object-cover" />
                      ) : (
                        <span className="w-full h-full flex items-center justify-center text-primary font-bold">{pickerBriefCampaign.brand[0]}</span>
                      )}
                    </div>
                    <div>
                      <div className="text-white/70 text-xs font-semibold uppercase tracking-wider mb-0.5">Your Assignment</div>
                      <h2 className="font-bold text-lg leading-tight">{pickerBriefCampaign.brand}</h2>
                      <p className="text-white/80 text-sm">{pickerBriefCampaign.project}</p>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2 pl-10">
                    <span className="bg-white/20 text-white text-xs font-bold px-3 py-1 rounded-full">{pickerBriefCampaign.rate}</span>
                    <span className="bg-white/20 text-white text-xs font-bold px-3 py-1 rounded-full">{pickerBriefCampaign.budget}</span>
                  </div>
                </div>

                {campaignBriefs[pickerBriefCampaign.id] && (
                  <div className="p-6 space-y-5 max-h-[55vh] overflow-y-auto">
                    <div>
                      ...
                      <p className="text-sm leading-relaxed">{campaignBriefs[pickerBriefCampaign.id].overview}</p>
                    </div>
                    <div>
                      <div className="flex items-center gap-2 mb-3 text-xs font-bold uppercase tracking-wider text-muted-foreground">
                        <Zap className="w-3.5 h-3.5 text-primary" /> 🎥 What To Create
                      </div>
                      <ul className="space-y-2">
                        {campaignBriefs[pickerBriefCampaign.id].whatToCreate.map((item, i) => (
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-3 text-xs font-bold uppercase tracking-wider text-muted-foreground">
                        <Target className="w-3.5 h-3.5 text-primary" /> ⚡ Video Structure
                      </div>
                      <div className="space-y-2">
                        {campaignBriefs[pickerBriefCampaign.id].structure.map((step, i) => (
                          <div key={i} className="flex gap-3 items-start bg-secondary/40 rounded-xl px-4 py-2.5">
                            <span className="w-5 h-5 rounded-full bg-primary text-white text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">{i + 1}</span>
                            <div>
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-2 text-xs font-bold uppercase tracking-wider text-muted-foreground">
                        <Megaphone className="w-3.5 h-3.5 text-primary" /> 📢 CTA
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {campaignBriefs[pickerBriefCampaign.id].cta.map((c, i) => (
                    ...
                    <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                      ...
                      {campaignBriefs[pickerBriefCampaign.id].rules.map((r, i) => (
                ...
                <div className="p-5 border-t border-border bg-secondary/20 flex gap-3">
                  <button onClick={() => setPickerBriefCampaign(null)} className="px-4 py-3 border border-border rounded-xl text-sm font-semibold hover:bg-secondary transition-colors">
                    Back
                  </button>
                  {pickerApplied.includes(pickerBriefCampaign.id) ? (
                    <button
                      onClick={() => setShowCampaignPicker(false)}
                      className="flex-1 py-3 bg-green-600 text-white font-bold rounded-xl text-sm flex items-center justify-center gap-2"
                    >
                      <CheckCircle2 className="w-4 h-4" /> Applied — Go to Dashboard
                    </button>
                  ) : (
                    <button
                      onClick={async () => {
                        await handlePickerApply(pickerBriefCampaign);
                        setShowCampaignPicker(false);
                      }}
                      disabled={pickerApplying === pickerBriefCampaign.id}
